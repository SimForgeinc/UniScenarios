 ...
 * back out of `scen batch`. Not part of the shipped library — this is how the
...
export function runBatch(doc, { maps = '--all-maps', draws = 1, tag = 'sweep' } = {}) {
  const file = `/tmp/scen-sweep-${tag}.json`;
  const out = `/tmp/scen-sweep-out/${tag}`;
  fs.rmSync(out, { recursive: true, force: true });
  fs.writeFileSync(file, JSON.stringify(doc));
  const args = ['batch', file, ...(maps === '--all-maps' ? ['--all-maps'] : ['--maps', maps]),
    '--draws', String(draws), '--out', out, '--no-trace'];
  const stdout = execFileSync('node', [SCEN, ...args], { encoding: 'utf8', maxBuffer: 1 << 28 });
