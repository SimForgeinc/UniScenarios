...
def job():
    return {
        ...
        "requiredSemantics": ["actor.trajectory", "traffic_signal.state", "collision.observe"],
        "frames": [
...
class BridgeTests(unittest.TestCase):
    def test_executes_one_backend_tick_per_authoritative_frame(self):
        ...
        self.assertEqual([call[0] for call in backend.calls], ["sync", "map", "freeze", "frame", "frame"])
    ...
    def test_rejects_unknown_semantics_and_non_contiguous_time(self):
        unknown = job()
        unknown["requiredSemantics"] = ["made.up.semantic"]
