...
function mockJob(id = 'mock-night-crossing', brief = 'A cyclist emerges from behind a stopped delivery van while an oncoming car yields late.') {
  ...
  return { jobId:id, brief, engine:'vista2', status:'complete', options:{ engine:'vista2', maps:['San Francisco','Boston'], ambient:'city' }, stages:stageDefs.map(([stage,name],i) => ({ stage:`${stage}-${name}`, status:'complete', elapsedMs:(i+1)*1380, artifacts:artifacts(stage), summary: stage === '50' ? { admitted:2, total:3 } : undefined })), cells:[
    { cellId:'sf-market-01-draw-0', map:'San Francisco', gate:{pass:true}, judge:{realism:8.4,dynamism:7.8,plausible:true}, artifacts:[{path:`${id}/60-render2d/cell-a/headline.png`,type:'image'},{path:`${id}/60-render2d/cell-a/rollout.mp4`,type:'video'}] },
    { cellId:'boston-seaport-04-draw-0', map:'Boston', gate:{pass:true}, judge:{realism:7.9,dynamism:8.2,plausible:true} },
    { cellId:'sf-market-01-draw-1', map:'San Francisco', gate:{pass:false,firstFailure:'C3 clearance below 5.0 m'}, judge:{realism:6.1,dynamism:7.2,plausible:true} },
  ] };
}
