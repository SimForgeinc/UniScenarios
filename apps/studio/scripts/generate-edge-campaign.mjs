...
function normalize(rawEntry, manifestFile, manifestOwner) {
  ...
  const diagnostics = [];
  if (!Number.isInteger(ordinal) || ordinal < 1 || ordinal > 10) diagnostics.push('ordinal must be an integer from 1 through 10');
  ...
  return {
    ...
    identity,
    ambient: ambient ? (ambientJson ? 'verified-evidence' : 'incomplete') : 'not-run',
    rubric: rubric && evidence ? 'verified-evidence' : rubric ? 'present-unverified' : 'missing',
...
console.log(`edge campaign: ${manifestFiles.length} manifests, ${entries.length}/10 scenarios`);
