...
describe('visual simulation agent', () => {
  it('sends a server-rendered PNG through the same high-effort graph', async () => {
    ...
    const args = generate.mock.calls[0]![0]; expect(args.reasoningEffort).toBe('high'); expect(args.images).toHaveLength(1); expect(args.images![0]!.dataUrl).toMatch(/^data:image\/png;base64,/u);
  });
});
