    ...
    for (const [identityKey, group] of [...byIdentity].sort(([a], [b]) => a.localeCompare(b))) {
      ...
      for (let part = 0; part < template.parts.length; part++) {
        ...
        list.forEach((actor, index) => {
          batch.mesh.setMatrixAt(index, poseMatrix(actor, template.dims));
          batch.mesh.setColorAt(index, instanceBodyTint(actor));
        });
...
function instanceBodyTint(actor: ActorView): Color {
  if (!actor.bodyColor) return new Color('#ffffff');
  const desired = new Color(actor.bodyColor);
  let source = new Color('#ffffff');
  try {
