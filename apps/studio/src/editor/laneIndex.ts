...
export class LaneIndex {
  ...
  static async load(url: string, options: LaneIndexOptions = {}): Promise<LaneIndex> {
    ...
    // The dev server hands the .gz through untouched; `decodeMaybeGzippedJson`
    // sniffs the magic number, so a host that inflates it also works.
    const json = await decodeMaybeGzippedJson<TopologyFile>(raw);
  ...
  private search(
    ...
    for (let r = 0; r <= rings; r++) {
      ...
      if (best && bestDist <= ((r - 1) * this.cellSize) ** 2) break;
