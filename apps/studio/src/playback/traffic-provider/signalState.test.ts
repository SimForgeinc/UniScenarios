...
describe('SUMO physical traffic-light state', () => {
  it('maps controlled links to retained OpenDRIVE head ids', () => {
    ...
    expect(topology.bindings.map(({ linkIndex, headIds }) => ({ linkIndex, headIds }))).toEqual([
  ...
  it('fits a complete red-to-green cycle inside a 20-second preview deterministically', () => {
    ...
    const durations = [...first.xml.matchAll(/<phase duration="([\d.]+)"/g)].map((match) => Number(match[1]));
    expect(durations.reduce((sum, duration) => sum + duration, 0)).toBeCloseTo(18, 1);
    expect(durations[1]).toBe(2);
    expect(durations[3]).toBe(2);
  });

  it('is safe for maps without controlled signals', () => {
    expect(parseSumoSignalTopology('<net><junction id="plain"/></net>')).toEqual({ bindings: [], mappedHeadIds: [] });
    expect(fitSumoSignalProgramsToScenario('<net/>', 20)).toEqual({ xml: '<net/>', adjustedControllers: 0 });
    expect(decodeSumoSignalSnapshot(new ArrayBuffer(0), 0, { bindings: [], mappedHeadIds: [] })).toEqual({
      heads: {}, mappedHeadCount: 0, unmappedLinkCount: 0,
    });
  });
});

function writeLink(view: DataView, index: number, hash: number, linkIndex: number, state: string): void {
