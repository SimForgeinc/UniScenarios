...
type SumoFactory = (options?: { noInitialRun?: boolean }) => Promise<SumoModule>;
...
async function handle(message: SumoWorkerRequest): Promise<void> {
  if (message.kind === 'init') {
    ...
    module = await imported.default({ noInitialRun: true });
  ...
  for (const actor of actors) {
    ...
    withString(sumo, actor.id, (idPointer) => withString(sumo, actor.routeId, (routePointer) => {
      assertOk(sumo._us_sumo_upsert_external(
        ...
        position.x,
        position.y,
        actor.headingDegrees - transform.rotationDegrees,
