 ...
 */
export function parseSumoSignalTopology(networkXml: string): SumoSignalTopology {
  const byLink = new Map<string, { controllerId: string; linkIndex: number; headIds: Set<string> }>();
  for (const match of networkXml.matchAll(/<tlLogic\b([^>]*)>([\s\S]*?)<\/tlLogic>/g)) {
    ...
    for (const param of match[2]!.matchAll(/<param\b([^>]*)\/?\s*>/g)) {
      ...
      addBinding(byLink, controllerId, linkIndex, decodeIds(attribute(param[1]!, 'value')));
    }
  }
  ...
  const bindings = [...byLink.values()]
    ...
    .sort((left, right) => left.controllerId.localeCompare(right.controllerId) || left.linkIndex - right.linkIndex);
  return {
    bindings,
    mappedHeadIds: [...new Set(bindings.flatMap((binding) => binding.headIds))].sort(),
  };
}
...
export function fitSumoSignalProgramsToScenario(
  ...
  const xml = networkXml.replace(/(<tlLogic\b[^>]*>)([\s\S]*?)(<\/tlLogic>)/g, (whole, open: string, body: string, close: string) => {
    ...
    if (phases.length < 2 || !(total > targetCycle) || phases.some((phase) => !(phase.duration > 0))) return whole;
    const clearance = phases.map((phase) => phase.state.includes('y') || phase.state.includes('Y') ? 2 : /^[rso]+$/i.test(phase.state) ? 1 : 0);
    ...
    const remaining = targetCycle - clearance.reduce<number>((sum, duration) => sum + duration, 0);
    if (!(activeTotal > 0) || remaining < active.filter((duration) => duration > 0).length * 2) return whole;
    let phaseIndex = 0;
    const nextBody = body.replace(/<phase\b([^>]*)\/?\s*>/g, (phaseXml: string) => {
      const index = phaseIndex++;
      const duration = clearance[index]! || remaining * active[index]! / activeTotal;
      return phaseXml.replace(/duration="[^"]*"/, `duration="${formatDuration(duration)}"`);
    });
...
export function decodeSumoSignalSnapshot(
  ...
  for (let index = 0; index < linkCount; index += 1) {
    ...
    links.set(`${controllerHash}:${linkIndex}`, phaseFromSumoState(view.getUint8(offset + 6)));
  }
  ...
  for (const [headId, phases] of candidates) heads[headId] = aggregatePhases(phases);
  return { heads, mappedHeadCount: Object.keys(heads).length, unmappedLinkCount: Math.max(0, linkCount - mappedLinks) };
}
...
function aggregatePhases(phases: readonly NormalizedTrafficLightPhase[]): NormalizedTrafficLightPhase {
...
function addBinding(
  bindings: Map<string, { controllerId: string; linkIndex: number; headIds: Set<string> }>,
  controllerId: string,
  linkIndex: number,
  ...
  const binding = bindings.get(key) ?? { controllerId, linkIndex, headIds: new Set<string>() };
  for (const headId of headIds) binding.headIds.add(headId);
  bindings.set(key, binding);
}

function decodeIds(value: string | undefined): string[] {
  return decodeXml(value ?? '').trim().split(/\s+/).filter(Boolean);
}
...
function formatDuration(value: number): string {
  return Math.max(.1, value).toFixed(2).replace(/\.00$/, '').replace(/(\.\d)0$/, '$1');
}
