...
import { buildMapSignalTimelineGroups, buildTimelineGroups, conflictingAction, editMapSignalPlanClip, editTimelineClipRange, interactionWithTimelineRange, TIMELINE_LAYOUT_EXTENSION_KEY, timelineLanePreferences, timelineLayoutExtension, moveMapSignalPlanClip, resizeMapSignalPlanClip, type TimelineActorGroup, type TimelineClipEditMode, type TimelineClipRange, type TimelineItem, type TimelineMapSignalClip, type TraceOutcomeMarker } from './model';
...
export function TimelineDock({ controller, editorState, session, outcomes = [], rightInset = 16, drawerMode = false, onActorInspect, onActorDelete, dashCameras = [], selectedDashCameraId = null, onDashCameraChange, onCameraPlay, onPlayPause, signalCatalog = null, signalControlDigest = null, selectedSignalHeadId = null, selectedSignalJunctionId = null, selectedSignalControllerId = null, selectedSignalResolved = true }: TimelineDockProps): JSX.Element {
  ...
  const groups = useMemo(() => {
    ...
    const lanes = { ...timelineLanePreferences(template.extensions), [clipPreview.interactionId]: clipPreview.lane };
    return buildTimelineGroups({ ...template, choreography: { ...template.choreography, interactions: template.choreography.interactions.map((item) => item.id === interaction.id ? interactionWithTimelineRange(item, clipPreview.range) : item) } }, lanes);
  ...
  const commitClipEdit = (item: TimelineItem, range: TimelineClipRange, lane: number): void => {
    if (!controller || readonly) return;
    const interaction = interactionWithTimelineRange(item.interaction, range);
    const lanes = { ...timelineLanePreferences(controller.doc.data.extensions), [item.interaction.id]: Math.max(0, Math.floor(lane)) };
...
function TimelineClip({ item, duration, lane, laneCount, readonly, selected, outcome, onSelect, onPreview, onCommit }: { item: TimelineItem; duration: number; lane: number; laneCount: number; readonly: boolean; selected: boolean; outcome: TimelineActionOutcome; onSelect: () => void; onPreview: (range: TimelineClipRange, lane: number) => void; onCommit: (range: TimelineClipRange, lane: number) => void }): JSX.Element {
  const dragged = useRef(false); const label = item.interaction.label ?? item.interaction.verb; const origin = { start: item.anchorTime, end: item.endTime };
  const beginDrag = (mode: TimelineClipEditMode) => (event: ReactMouseEvent<HTMLButtonElement>): void => {
    if (readonly || event.button !== 0) return; event.preventDefault(); event.stopPropagation();
    ...
    const finish = (pointer: MouseEvent): void => { window.removeEventListener('mousemove', update); if (dragged.current || Math.abs(pointer.clientX - startX) > 2 || Math.abs(pointer.clientY - startY) > 2) { update(pointer); onCommit(latestRange, latestLane); } };
    window.addEventListener('mousemove', update); window.addEventListener('mouseup', finish, { once: true });
  };
  const keyboardEdit = (mode: TimelineClipEditMode) => (event: ReactKeyboardEvent<HTMLButtonElement>): void => {
    if (readonly) return; const amount = event.shiftKey ? 1 : .1;
  ...
  return <div title={`${label} · eligibility ${item.anchorTime.toFixed(2)}–${item.endTime.toFixed(2)}s${maneuver} · ${outcome}`} data-outcome={outcome} data-testid={`timeline-item-${item.interaction.id}`} data-lane={lane} style={{ ...styles.clip, ...(outcome === 'executed' ? styles.clipExecuted : outcome === 'missed' ? styles.clipMissed : null), left: `${item.anchorTime / duration * 100}%`, width: `${Math.max(3, (item.endTime - item.anchorTime) / duration * 100)}%`, ...(selected ? styles.clipSelected : null) }}>
    <button type="button" aria-label={`Resize start of ${label}`} style={{ ...styles.resizeHandle, ...styles.resizeStart }} onMouseDown={beginDrag('resize-start')} onKeyDown={keyboardEdit('resize-start')} disabled={readonly} data-testid={`timeline-resize-start-${item.interaction.id}`} />
    <button type="button" aria-label={`Edit and move ${label}`} style={styles.clipBody} onMouseDown={beginDrag('move')} onKeyDown={keyboardEdit('move')} onClick={() => { if (!dragged.current) onSelect(); dragged.current = false; }} disabled={readonly}>{label}</button>
    <button type="button" aria-label={`Resize end of ${label}`} style={{ ...styles.resizeHandle, ...styles.resizeEnd }} onMouseDown={beginDrag('resize-end')} onKeyDown={keyboardEdit('resize-end')} disabled={readonly} data-testid={`timeline-resize-end-${item.interaction.id}`} />
  </div>;
}
