...
export type TimelineActionDraft = Omit<ActionEditorState, 'maneuverDuration' | 'maneuverStyle' | 'sourceDefinitionId' | 'timingEditable' | 'triggerMode' | 'triggerActorId' | 'triggerThreshold' | 'triggerDeadline' | 'triggerIfNever' | 'desiredOutcome' | 'nearMissClearanceM' | 'nearMissPass' | 'nearMissMinSpeedMps' | 'nearMissMaxSpeedMps'> & {
  ...
  triggerMode?: ActorRelativeTriggerMode;
  triggerActorId?: string;
...
export function submitTimelineAction(
  ...
  try {
    const triggerMode = draft.triggerMode ?? 'time';
    const triggerActorId = draft.triggerActorId ?? '';
    const triggerThreshold = draft.triggerThreshold ?? (triggerMode === 'ttc' ? 2 : 12);
    const triggerDeadline = draft.triggerDeadline ?? 20;
    const triggerIfNever = draft.triggerIfNever ?? 'skip';
    ...
    if (triggerMode !== 'time') {
      const target = template.roles.find((item) => item.id === triggerActorId);
      if (!target) return { ok: false, message: 'The trigger actor no longer exists. Choose another authored actor.' };
      if (target.id === draft.actorId) return { ok: false, message: 'An actor cannot trigger itself.' };
...
function actorRelativeTriggerState(interaction: Interaction): Pick<ActionEditorState, 'triggerMode' | 'triggerActorId' | 'triggerThreshold' | 'triggerDeadline' | 'triggerIfNever'> {
...
export function TimelineDock({ controller, editorState, session, outcomes = [], rightInset = 16, drawerMode = false, onActorInspect, onActorDelete, dashCameras = [], selectedDashCameraId = null, onDashCameraChange, onCameraPlay, onPlayPause, signalCatalog = null, signalControlDigest = null, selectedSignalHeadId = null, selectedSignalJunctionId = null, selectedSignalControllerId = null, selectedSignalResolved = true }: TimelineDockProps): JSX.Element {
  ...
  return <section style={{ ...styles.dock, width: drawerMode && !expanded ? 124 : '100%', height: drawerMode && !expanded ? 44 : '100%' }} data-testid="timeline-dock" data-layout={drawerMode ? 'drawer' : 'sidebar'}>
    ...
    {expanded ? <div style={styles.timelineBody} data-testid="timeline-full-width"><div style={styles.canvas}><Ruler duration={duration} />
        {signalGroups.map((group) => { const issue = signalBindingIssues.get(group.planId); return <div key={`${group.planId}:track`} style={{ ...styles.track, ...styles.identityTrack }} onDoubleClick={() => { openSignalEditor(group.plan, undefined, group.plan.clips[0]?.reference); if (group.plan.clips.length === 0) setNotice('Select a physical signal orb before adding the first phase clip.'); }} data-testid={`timeline-signal-${group.planId}`}><div style={{ ...styles.identitySticker, ...styles.signalSticker }} data-testid={`timeline-signal-sticker-${group.planId}`}><span>🚦 {group.label}{issue ? <small role="alert"> · Stale binding</small> : null}</span>{!readonly ? <button type="button" style={styles.stickerButton} aria-label={`Delete ${group.label} signal plan`} onClick={(event) => { event.stopPropagation(); controller?.doc.removeMapSignalPlan(group.planId); }}>🗑</button> : null}<button type="button" style={styles.stickerButton} disabled={readonly || Boolean(issue)} onClick={(event) => { event.stopPropagation(); openSignalEditor(group.plan); }} aria-label={`Add phase for ${group.label}`} data-testid={`timeline-add-signal-${group.planId}`}>＋</button></div>{group.clips.map((item) => <SignalTimelineClip key={item.clip.id} item={item} duration={duration} readonly={readonly || Boolean(issue)} selected={selectedSignalClip === item.clip.id} onSelect={() => openSignalEditor(group.plan, item.clip)} onMove={(time) => applySignalEdit(moveMapSignalPlanClip(group.plan, item.clip.id, time, duration))} onResize={(edge, time) => applySignalEdit(resizeMapSignalPlanClip(group.plan, item.clip.id, edge, time, duration))} />)}</div>; })}
...
function TimelineClip({ item, duration, lane, laneCount, identityLane, readonly, selected, outcome, onSelect, onPreview, onCancelPreview, onCommit }: { item: TimelineItem; duration: number; lane: number; laneCount: number; identityLane: boolean; readonly: boolean; selected: boolean; outcome: TimelineActionOutcome; onSelect: () => void; onPreview: (range: TimelineClipRange, lane: number) => void; onCancelPreview: () => void; onCommit: (range: TimelineClipRange, lane: number) => void }): JSX.Element {
  ...
  const beginDrag = (mode: TimelineClipEditMode) => (event: ReactMouseEvent<HTMLButtonElement>): void => {
    ...
    const laneAt = (clientY: number): number => {
      const siblings = [...(track.parentElement?.querySelectorAll<HTMLElement>('[data-actor-id][data-lane-index]') ?? [])]
        .filter((candidate) => candidate.dataset['actorId'] === item.actorId);
      const last = siblings.at(-1);
      if (last && clientY > last.getBoundingClientRect().bottom) return laneCount;
      let closest = lane; let distance = Number.POSITIVE_INFINITY;
      for (const candidate of siblings) { const rect = candidate.getBoundingClientRect(); const next = Math.abs(clientY - (rect.top + rect.bottom) / 2); if (next < distance) { distance = next; closest = Number(candidate.dataset['laneIndex']); } }
      return clamp(closest, 0, laneCount);
    };
    const update = (pointer: MouseEvent): void => { latestRange = editTimelineClipRange(origin, mode, (pointer.clientX - startX) / Math.max(1, bounds.width) * duration, duration); latestLane = mode === 'move' ? laneAt(pointer.clientY) : lane; dragged.current ||= Math.abs(pointer.clientX - startX) > 2 || Math.abs(pointer.clientY - startY) > 2; onPreview(latestRange, latestLane); };
...
const styles: Record<string, CSSProperties> = { dock: { position: 'relative', zIndex: 18, overflow: 'hidden', background: 'rgba(23,25,29,.99)', borderRight: '1px solid #3a3e46', color: '#e8ebf0' }, transport: { minHeight: 44, display: 'flex', alignItems: 'center', gap: 7, padding: '0 8px', borderBottom: '1px solid #343841' }, timelineTitle: { fontSize: 11 }, transportButton: { width: 32, height: 28, borderRadius: 5, borderWidth: 1, borderStyle: 'solid', borderColor: '#c66b2c', background: '#8d451b', color: '#fff' }, cameraPlayButton: { borderColor: '#4388c7', background: '#245b89' }, cameraSelect: { maxWidth: 130, background: '#272c33', color: '#eee' }, noCamera: { color: '#788391', fontSize: 9 }, time: { fontSize: 10, whiteSpace: 'nowrap' }, scrubber: { minWidth: 55, flex: 1 }, collapse: { background: '#292d34', color: '#eee', border: 0 }, notice: { padding: 8, background: '#4c371b', color: '#ffd89a', fontSize: 10 }, error: { padding: 6, background: '#471d24', color: '#ff9b9b' }, worldTrack: { minHeight: 28, display: 'grid', gridTemplateColumns: '48px 1fr auto', padding: '5px 8px', fontSize: 9 }, signalSelection: { minHeight: 32, display: 'flex', alignItems: 'center', gap: 6, padding: '4px 8px', borderBottom: '1px solid #343841', color: '#d8c995', fontSize: 9 }, timelineBody: { height: 'calc(100% - 72px)', minHeight: 180, overflow: 'auto' }, actorLabel: { flex: 1, minWidth: 0, height: 25, padding: '0 7px', border: 0, background: 'transparent', color: 'inherit', display: 'flex', alignItems: 'center', gap: 5 }, actorName: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }, actorSelected: { background: '#354b69', borderColor: '#5f88bd' }, canvas: { position: 'relative', minWidth: 248 }, ruler: { position: 'relative', height: 24, background: '#1c1f24', borderBottom: '1px solid #424751' }, tick: { position: 'absolute', top: 4, fontSize: 9, color: '#87909d' }, track: { position: 'relative', height: 44, borderBottom: '1px solid #2c3037' }, identityTrack: { height: 68, background: 'rgba(255,255,255,.018)' }, identitySticker: { position: 'absolute', zIndex: 4, top: 4, maxWidth: 'calc(100% - 8px)', height: 27, boxSizing: 'border-box', display: 'flex', alignItems: 'center', border: '1px solid #46505d', borderRadius: 999, background: '#292d34', color: '#e8ebf0', fontSize: 10, overflow: 'hidden' }, actorSticker: { right: 4 }, signalSticker: { left: 4, paddingLeft: 8, borderColor: '#665f3d', background: '#343123', color: '#eadc9d', gap: 4 }, stickerButton: { flex: '0 0 auto', width: 25, height: 25, padding: 0, border: 0, borderLeft: '1px solid rgba(255,255,255,.12)', background: 'transparent', color: 'inherit' }, clip: { position: 'absolute', top: 10, height: 24, minWidth: 16, display: 'flex', overflow: 'visible', borderWidth: 1, borderStyle: 'solid', borderColor: '#68a8ed', borderRadius: 4, background: '#3276c8', color: '#fff', fontSize: 9, whiteSpace: 'nowrap' }, identityClip: { top: 37 }, clipBody: { flex: 1, minWidth: 0, padding: '0 7px', overflow: 'hidden', border: 0, background: 'transparent', color: 'inherit', cursor: 'grab', fontSize: 9, textOverflow: 'ellipsis', whiteSpace: 'nowrap' }, signalClip: { position: 'absolute', top: 36, height: 28, minWidth: 18, boxSizing: 'border-box', borderWidth: 1, borderStyle: 'solid', borderRadius: 4, color: '#fff' }, signalClipBody: { position: 'absolute', inset: '0 6px', overflow: 'hidden', border: 0, background: 'transparent', color: 'inherit', fontSize: 9, whiteSpace: 'nowrap', textOverflow: 'ellipsis' }, resizeHandle: { position: 'absolute', zIndex: 2, top: -1, bottom: -1, width: 8, padding: 0, border: 0, borderRadius: 3, background: 'rgba(255,255,255,.3)', cursor: 'ew-resize' }, resizeStart: { left: -1, borderRadius: '4px 0 0 4px' }, resizeEnd: { right: -1, borderRadius: '0 4px 4px 0' }, clipExecuted: { background: '#287a4c', borderColor: '#67d99a' }, clipMissed: { background: '#71313a', borderColor: '#ff788c' }, clipSelected: { outline: '2px solid #fff' }, outcome: { position: 'absolute', top: 4, width: 3, height: 36, background: '#ff5e7a' }, identityOutcome: { top: 32 }, playhead: { position: 'absolute', zIndex: 5, top: 0, bottom: 0, width: 1, background: '#ff8551', pointerEvents: 'none' }, editor: { position: 'fixed', zIndex: 32, bottom: 104, width: 340, padding: 14, borderRadius: 8, background: '#22262d', border: '1px solid #555c68', fontSize: 10 }, editorHeader: { display: 'flex', justifyContent: 'space-between', marginBottom: 10, fontSize: 13 }, editorContext: { color: '#9ba4b2', fontSize: 10 }, close: { border: 0, background: 'transparent', color: '#eee', fontSize: 18 }, field: { display: 'grid', gridTemplateColumns: '80px 1fr', gap: 8, alignItems: 'center', marginBottom: 8, color: '#aab3c1' }, resourceHint: { padding: 7, borderRadius: 4, background: '#29333f', color: '#9eb8d3' }, editorActions: { display: 'flex', gap: 6, marginTop: 10 }, save: { flex: 1, padding: 8, background: '#286aa8', color: '#fff', border: 0, borderRadius: 5 }, delete: { padding: 8, background: '#52252c', color: '#ffabb8', border: 0, borderRadius: 5 } };
