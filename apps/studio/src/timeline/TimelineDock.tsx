...
export interface TimelineDockProps {
  ...
  rightInset?: number;
  drawerMode?: boolean;
  ambientSummary?: { enabled: boolean; busy: boolean; count: number; preset: string };
...
export function TimelineDock({ controller, editorState, session, outcomes = [], achievedSpeeds = {}, rightInset = 16, drawerMode = false, ambientSummary, onActorInspect, onActorDelete, dashCameras = [], selectedDashCameraId = null, onDashCameraChange, onCameraPlay, onPlayPause }: TimelineDockProps): JSX.Element {
  ...
  return <section style={{ ...styles.dock, width: drawerMode && !expanded ? 124 : '100%', height: drawerMode && !expanded ? 44 : '100%' }} data-testid="timeline-dock" data-layout={drawerMode ? 'drawer' : 'sidebar'}>
    ...
    {expanded && ambientSummary ? <div style={{ ...styles.worldTrack, ...(ambientSummary.enabled ? styles.worldTrackActive : null) }} data-testid="timeline-ambient-summary"><strong>World</strong><span>Ambient traffic</span><span>{ambientSummary.busy ? 'generating…' : ambientSummary.enabled ? `${ambientSummary.count} · ${ambientSummary.preset}` : 'off'}</span></div> : null}
