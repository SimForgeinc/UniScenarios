...
export function GenerationsWorkspace({ currentMapId, currentMapHash, onOpenDraft, onSwitchMap, onClose }: GenerationsWorkspaceProps): JSX.Element {
  ...
  return <section style={styles.workspace} aria-label="Scenario generations" data-testid="generations-workspace">
    ...
    <div style={styles.filters}>
      <label style={styles.searchLabel}>Search<input autoFocus type="search" value={query} onChange={(event) => setQuery(event.currentTarget.value)} placeholder="Prompt, scenario, model, or map" style={styles.search} /></label>
      <label>Method<select value={method} onChange={(event) => setMethod(event.currentTarget.value)}><option value="all">All methods</option>{methods.map((item) => <option key={item} value={item}>{providerName(item)}</option>)}</select></label>
      <label>Map<select value={mapFilter} onChange={(event) => setMapFilter(event.currentTarget.value)}><option value="all">All maps</option>{maps.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
      <label>Status<select value={status} onChange={(event) => setStatus(event.currentTarget.value)}><option value="all">All saved drafts</option><option value="verified">Simulation verified</option><option value="matches">Matches request</option><option value="needs-review">Needs semantic review</option></select></label>
    ...
    {error ? <div role="alert" style={styles.error}><strong>Could not load generations</strong><span>{error}</span><button type="button" onClick={load}>Try again</button></div> : null}
...
function GenerationGalleryCard({ entry, currentMapId, currentMapHash, onOpen, onSwitch, onDetails }: { entry: CopilotGenerationHistoryEntry; currentMapId: string; currentMapHash: string | null; onOpen: () => void; onSwitch: () => void; onDetails: () => void }): JSX.Element {
  ...
  return <article role="button" tabIndex={compatibility.compatible ? 0 : -1} aria-disabled={!compatibility.compatible} aria-label={`Open saved scenario ${entry.caseTitle}`} onClick={open} onKeyDown={keyDown} style={{ ...styles.card, ...(compatibility.compatible ? styles.cardOpen : styles.cardBlocked) }} data-testid="generation-draft-card">
    ...
    <div style={styles.cardBody}>
      ...
      <div style={styles.actions}>
        {!compatibility.compatible && compatibility.switchable ? <button type="button" style={styles.switch} onClick={(event) => { event.stopPropagation(); onSwitch(); }}>Switch to {entry.mapId}</button> : null}
        <button type="button" style={styles.detailsButton} onClick={(event) => { event.stopPropagation(); onDetails(); }}>View details</button>
      </div>
    </div>
  </article>;
}
...
const styles: Record<string, CSSProperties> = {
  workspace: { position: 'fixed', inset: '42px 0 0', zIndex: 80, overflowY: 'auto', background: '#101318', color: '#eef3f8', padding: 'clamp(16px,3vw,34px)', font: '13px/1.45 ui-sans-serif,system-ui,sans-serif' },
  header: { display: 'flex', justifyContent: 'space-between', gap: 20, alignItems: 'flex-start', maxWidth: 1500, margin: '0 auto 22px' }, eyebrow: { color: '#70cfff', fontSize: 10, letterSpacing: '.13em', fontWeight: 800 }, title: { margin: '3px 0', fontSize: 30 }, subtitle: { margin: 0, color: '#9eabb8' }, close: { border: 0, background: 'transparent', color: '#e8eef5', fontSize: 28, cursor: 'pointer' },
  filters: { maxWidth: 1500, margin: '0 auto 18px', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'end' }, searchLabel: { flex: '1 1 280px' }, search: { width: '100%', boxSizing: 'border-box' }, count: { marginLeft: 'auto', color: '#9ca8b5', paddingBottom: 7 },
  grid: { maxWidth: 1500, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(min(330px,100%),1fr))', gap: 14 }, card: { overflow: 'hidden', borderRadius: 12, background: '#1b2027', border: '1px solid #343d48', minWidth: 0, outlineOffset: 3 }, cardOpen: { cursor: 'pointer' }, cardBlocked: { opacity: .72 }, preview: { height: 150, position: 'relative', background: '#111820', overflow: 'hidden' }, image: { width: '100%', height: '100%', objectFit: 'cover' }, previewFallback: { height: '100%', display: 'grid', placeContent: 'center', textAlign: 'center', color: '#6d8497', fontSize: 32 }, mapBadge: { position: 'absolute', top: 9, right: 9, borderRadius: 999, padding: '4px 8px', background: 'rgba(10,15,20,.84)', fontSize: 10, color: '#cde8ff' }, cardBody: { padding: 13, display: 'flex', flexDirection: 'column', gap: 8 }, cardTop: { display: 'flex', gap: 8, justifyContent: 'space-between', alignItems: 'center' }, method: { color: '#89d9ff', fontWeight: 750, fontSize: 11 }, status: { borderRadius: 999, padding: '3px 7px', fontSize: 9, fontWeight: 800 }, good: { background: '#163b28', color: '#7de5a9' }, warn: { background: '#45361e', color: '#ffd076' }, bad: { background: '#472329', color: '#ff9ea7' }, neutral: { background: '#303944', color: '#b9c4d0' }, cardTitle: { margin: 0, fontSize: 17 }, prompt: { color: '#bdc6d0', margin: 0, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }, meta: { display: 'flex', flexWrap: 'wrap', gap: 8, color: '#8f9ba8', fontSize: 10 }, compatibility: { padding: 8, background: '#382a22', color: '#ffc890', borderRadius: 6 }, openHint: { color: '#7edbb0', fontWeight: 800 }, actions: { display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 2 }, detailsButton: { border: '1px solid #566373', background: '#242b33', color: '#eef4fb', borderRadius: 6, padding: '7px 10px' }, switch: { border: '1px solid #3d7893', background: '#183445', color: '#bfeaff', borderRadius: 6, padding: '7px 10px' },
  state: { maxWidth: 1500, margin: '70px auto', display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'center', color: '#9cabb8' }, error: { maxWidth: 1500, margin: '10px auto', padding: 12, background: '#422329', color: '#ffc1c6', borderRadius: 8, display: 'flex', flexDirection: 'column', gap: 5 }, backdrop: { position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(3,7,11,.72)', display: 'flex', justifyContent: 'flex-end' }, drawer: { width: 'min(680px,94vw)', height: '100%', boxSizing: 'border-box', overflowY: 'auto', padding: 20, background: '#171c22', borderLeft: '1px solid #45515e' }, drawerHeader: { display: 'flex', justifyContent: 'space-between', gap: 12 }, detailGrid: { display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 8 }, pre: { maxHeight: 300, overflow: 'auto', whiteSpace: 'pre-wrap', background: '#0d1116', padding: 10, borderRadius: 6 }, checkGood: { color: '#7fe0a7', margin: '4px 0' }, checkBad: { color: '#ffae82', margin: '4px 0' },
};
