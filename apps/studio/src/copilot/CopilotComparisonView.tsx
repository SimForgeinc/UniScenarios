...
function GenerationCard({ entry, onRerun, onApply, onInspect, currentMapId, currentMapHash }: { entry: CopilotGenerationHistoryEntry; onRerun: Props['onRerun']; onApply: Props['onApply']; onInspect: (preview: boolean) => void; currentMapId: string; currentMapHash: string | null }): JSX.Element {
  ...
  return <article style={{ ...styles.card, ...(requested ? styles.cardMatch : runnable ? styles.cardMismatch : styles.cardFailed) }} data-testid="copilot-generation-card">
    ...
    <div style={styles.meta}>{entry.actualModel ?? 'model not recorded'} · {entry.reasoningEffort} effort · {entry.mapId} · seed {entry.seed ?? 'not recorded'}</div>
...
const styles: Record<string, CSSProperties> = {
  registry: { padding: 10, border: '1px solid #3b5060', borderRadius: 8, background: '#18242c', color: '#c5ddea' }, experimentGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 7, paddingTop: 8 }, experiment: { display: 'flex', flexDirection: 'column', gap: 4, padding: 8, background: '#202d35', borderRadius: 6, fontSize: 10 },
