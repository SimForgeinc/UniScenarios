...
describe('first-run graphics chooser', () => {
  it('renders an accessible blocking decision with three exact options and no preselected value', () => {
    const markup = renderToStaticMarkup(<FirstRunGraphicsChooser onChoose={() => undefined} />);
    expect(markup).toContain('role="dialog"');
    expect(markup).toContain('aria-modal="true"');
    expect(markup.match(/data-testid="graphics-choice-/g)).toHaveLength(3);
