...
`preferred` invariant emits nothing at all: 18 cells were ACCEPTED with violated `preferred` invariants.
...
whether or not you declare the relation. So declare `occludes` only when the hidden reveal is the entire
point of the scenario AND you have put the occluder squarely between the ego's approach and the target's
conflict point, close to the target and large enough to hide it (`vehicle.box_truck`, `vehicle.bus`,
`vehicle.semi_truck`). Otherwise place the prop and say nothing.
