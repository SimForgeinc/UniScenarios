...
export const V2_SCHEMA_BASE = 'https://scenario-studio.simforge.ai/schema';
...
export function buildTemplateJsonSchema(): Record<string, unknown> {
  return build(
    ...
    `Scenario Studio scenario template, schema v${SCENARIO_TEMPLATE_VERSION}`,
...
export function buildAnchorJsonSchema(): Record<string, unknown> {
  return build(
    ...
    'Scenario Studio logical anchor, schema v2',
...
export function buildInteractionsJsonSchema(): Record<string, unknown> {
  return build(
    ...
    'Scenario Studio interaction list, schema v2',
