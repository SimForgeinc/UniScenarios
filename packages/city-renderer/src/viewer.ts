...
export class CityViewer {
  ...
  private residency(): {
    ...
    // A lost context counts as work in flight: the recovery path rebuilds the
    // scene through runPresetTransition, and until it does no residency counter
    // can be trusted to describe drawable state.
    let loading = Number(this.mapLoadActive) + Number(this.contextLost)
