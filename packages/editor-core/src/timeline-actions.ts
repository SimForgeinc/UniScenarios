...
export type ActionResource = 'longitudinal' | 'lateral' | 'topology' | 'indicator' | 'horn' | 'lights';
...
const VEHICLE: readonly ActionDefinition[] = [
  speed('accelerate', 'Accelerate', { mode: 'delta', deltaKph: 10 }), speed('decelerate', 'Decelerate', { mode: 'delta', deltaKph: -10 }), speed('brake_stop', 'Brake to stop', { mode: 'stop' }, 1.5), speed('resume', 'Resume default speed', { mode: 'resume' }), speed('target_speed', 'Set target speed', { mode: 'absolute', valueKph: 48 }),
  lateral('lane_left', 'Change lane left', 'changeLane', { mode: 'relative', dk: 1 }), lateral('lane_right', 'Change lane right', 'changeLane', { mode: 'relative', dk: -1 }), route('keep_lane', 'Go straight / keep lane', 'straight'), route('turn_left', 'Turn left', 'left'), route('turn_right', 'Turn right', 'right'), customRoute(), lateral('pull_over', 'Pull over', 'laneOffset', { tFrac: -.8, reference: 'lane_center' }, 3),
...
export function actionResource(interaction: Interaction): ActionResource | null { if (interaction.verb === 'speed' || interaction.verb === 'gap') return 'longitudinal'; if (interaction.verb === 'changeLane' || interaction.verb === 'laneOffset') return 'lateral'; if (interaction.verb === 'route') return 'topology'; if (interaction.verb !== 'set') return null; const key = String(interaction.target.key); if (key === 'lights.indicator') return 'indicator'; if (key === 'audio.horn') return 'horn'; if (key === 'lights.headlights') return 'lights'; return null; }
