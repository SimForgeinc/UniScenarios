...
const map = {
  runtimeAssetId: 'usmv_test',
  mapVersionId: 'usmv_test',
  sourceMapId: 'test-map_20260805-010203',
...
describe('worker-side MapRuntime identity', () => {
  it('uses sourceMapId for semantic map identity when the immutable version differs', () => {
    expect(map.runtimeAssetId).toBe(map.mapVersionId);
    expect(runtime.mapId).toBe(map.sourceMapId);
    expect(runtime.mapId).not.toBe(map.mapVersionId);
  });
  ...
  it('is deterministic for warm reuse and invalidates on every map artifact class', () => {
    ...
    expect(mapAssetDigest({ ...map, browserClosureSha256: 'b'.repeat(64) })).not.toBe(mapAssetDigest(map));
    expect(mapAssetDigest({ ...map, mapVersionId: 'usmv_republished' })).not.toBe(mapAssetDigest(map));
